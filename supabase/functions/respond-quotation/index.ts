...
Deno.serve(async (request) => {
  ...
  if (!url || !serviceKey || !authorization) return new Response(JSON.stringify({ error: "Unauthorized" }), { status: 401 });
  ...
  if (!body.notification_id || !["accept", "reject"].includes(body.action)) return new Response(JSON.stringify({ error: "Invalid response" }), { status: 400 });
  ...
  return new Response(JSON.stringify({ quotation: data }), { status: 200, headers: { "Content-Type": "application/json" } });
});

...
Deno.serve(async (request) => {
  ...
  const profile = await service.from("profiles").select("role").eq("id", actor.data.user.id).maybeSingle(); if (body.recipient_profile_id !== actor.data.user.id && profile.data?.role !== "admin") return new Response(JSON.stringify({ error: "Forbidden" }), { status: 403 });

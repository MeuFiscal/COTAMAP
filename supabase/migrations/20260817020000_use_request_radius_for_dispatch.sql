-- Dispatch must respect the radius selected on each quote request.
-- Keep the legacy three-argument entry point as a compatibility wrapper.
...
begin
  ...
  search_radius := least(greatest(coalesce(target_radius_meters, settings.initial_radius_meters), 1), settings.max_radius_meters);
  ...
  with eligible as (
    select b.id, b.name, b.logo_url, b.latitude, b.longitude,
      ...
      count(e.id) filter (where e.is_active and e.presence_status = 'online' and e.last_activity_at >= now() - interval '3 minutes') as online_operators
    ...
    having count(e.id) filter (where e.is_active and e.presence_status = 'online' and e.last_activity_at >= now() - interval '3 minutes') > 0
  ), scored as (
    select eligible.*, (
...
as $$
  select * from public.buscar_empresas_inteligente(
    target_latitude, target_longitude, target_category_id,
    (select initial_radius_meters from public.distribution_settings where id = true)
  );
$$;

revoke all on function public.buscar_empresas_inteligente(numeric, numeric, uuid, integer) from public, anon, authenticated;
grant execute on function public.buscar_empresas_inteligente(numeric, numeric, uuid, integer) to service_role;
...
begin
  ...
  select * into settings from public.distribution_settings where id = true;
  for candidate in
    select * from public.buscar_empresas_inteligente(request_row.latitude, request_row.longitude, request_row.business_category_id, request_row.radius_meters)
    order by score desc, distance_meters asc
    limit settings.initial_business_count
  loop
...
begin
  ...
  from public.buscar_empresas_inteligente(request_row.latitude, request_row.longitude, request_row.business_category_id, request_row.radius_meters) eligible
  ...
  order by eligible.score desc, eligible.distance_meters asc
  limit 1;

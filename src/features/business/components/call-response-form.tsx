...
export function CallResponseForm({ notificationId, businessId }: { notificationId: string; businessId: string }) {
  ...
  return <form onSubmit={handleSubmit((values) => mutation.mutate(values))} className="space-y-5 rounded-[2rem] bg-[#FFFFFF] p-6 shadow-sm sm:p-8" noValidate>
    ...
    <label className="block text-sm font-bold">Preço<input type="number" step="0.01" min="0" {...register("amount")} className="mt-2 w-full rounded-xl border border-[#111827]/10 p-3" />{errors.amount ? <span className="text-xs text-[#C2410C]">{errors.amount.message}</span> : null}</label>

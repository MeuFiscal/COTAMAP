...
export function CallResponseForm({ notificationId, businessId, items = [] }: { notificationId: string; businessId: string; items?: RequestedItem[] }) {
  ...
  const [itemResponses, setItemResponses] = useState(() => items.map((item) => ({ quote_request_item_id: item.id, available: true, unit_price: 0, quantity: item.quantity, notes: "" })));

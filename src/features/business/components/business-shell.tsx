...
export function BusinessShell({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const callsChannel = supabase.channel(`business-call-alerts-${business?.id ?? operator.id}`).on("postgres_changes", { event: "INSERT", schema: "public", table: "quote_notifications", filter: business?.id ? `business_id=eq.${business.id}` : undefined }, (payload) => { const id = typeof payload.new.id === "string" ? payload.new.id : null; if (id) { playCallAlert(); setNewCall({ id }); } }).subscribe();
  ...
  const playCallAlert = () => {
    try {
      ...
      // O navegador pode bloquear áudio até a primeira interação do operador.
    }
  };

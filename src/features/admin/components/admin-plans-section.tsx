...
import { ChevronDown, Edit3, MoreHorizontal, Plus, X } from "lucide-react";
...
function PlanEditor({ plan, onClose, update }: { plan?: Plan; onClose: () => void; update: Props["update"] }) {
  ...
  const submit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const body: Record<string, unknown> = {
      ...
      price: Number(f.get("price")), promotional_price: f.get("promotional_price") ? Number(f.get("promotional_price")) : null,
  ...
  return <div className="fixed inset-0 z-50 flex justify-end bg-slate-950/30 backdrop-blur-[2px]" role="dialog" aria-modal="true" aria-label={plan ? `Editar ${plan.name}` : "Novo plano"}>
    <div className="h-full w-full max-w-xl overflow-y-auto bg-[#f8fafc] shadow-2xl">
      ...
      <form onSubmit={submit} className="space-y-7 px-6 py-7 sm:px-8">
        <section><h3 className="text-sm font-black text-slate-950">Informações do plano</h3><div className="mt-4 grid gap-4"><label className={labelClass}>Código interno<input name="code" defaultValue={plan?.code} required pattern="[a-z0-9]+([-_][a-z0-9]+)*" className={inputClass}/></label><label className={labelClass}>Nome<input name="name" defaultValue={plan?.name} required className={inputClass}/></label><label className={labelClass}>Descrição<textarea name="description" defaultValue={plan?.description} rows={2} className={inputClass}/></label></div></section>
        <section><h3 className="text-sm font-black text-slate-950">Preço</h3><div className="mt-4 grid gap-4 sm:grid-cols-2"><label className={labelClass}>Preço mensal<input name="price" defaultValue={plan?.price} required type="number" min="0" step="0.01" className={inputClass}/></label><label className={labelClass}>Preço promocional <span className="font-normal text-slate-400">(opcional)</span><input name="promotional_price" defaultValue={plan?.promotional_price ?? ""} type="number" min="0" step="0.01" className={inputClass}/></label></div></section>
        ...
        <section><h3 className="text-sm font-black text-slate-950">Integração Cakto</h3><div className="mt-4 grid gap-4 rounded-2xl border border-orange-100 bg-orange-50/40 p-4"><div className="flex items-end gap-3"><label className={`${labelClass} flex-1`}>Produto Cakto<select value={productId} onChange={e => void loadOffers(e.target.value)} className={inputClass}><option value="">Selecione um produto</option>{products.map(product => <option key={product.id} value={product.id}>{product.name} · {product.price === 0 ? "Grátis" : product.price == null ? "Preço não informado" : `R$ ${product.price.toFixed(2).replace(".", ",")}`} · {product.type || "Produto"}</option>)}</select></label><button type="button" onClick={() => void refreshProducts()} disabled={catalogLoading} className="mb-0 rounded-xl border border-orange-200 bg-white px-3 py-3 text-xs font-bold text-orange-700">{catalogLoading ? "Atualizando..." : "Atualizar da Cakto"}</button></div><label className={labelClass}>Oferta<select value={offerId} onChange={e => setOfferId(e.target.value)} disabled={!productId || catalogLoading} className={inputClass}><option value="">Selecione uma oferta</option>{offers.map(offer => <option key={offer.id} value={offer.id}>{offer.name} · {offer.price == null ? "Preço não informado" : `R$ ${offer.price.toFixed(2).replace(".", ",")}`} · {offer.recurrence_period ? `a cada ${offer.recurrence_period} dias` : offer.type || "Oferta"}</option>)}</select></label><p className="text-[11px] text-orange-800/70">Os IDs serão associados automaticamente. A URL do checkout continua sendo preenchida manualmente na seção avançada.</p></div></section>
        <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white"><button type="button" onClick={() => setAdvanced(v => !v)} className="flex w-full items-center justify-between px-4 py-4 text-left text-sm font-black text-slate-800"><span>Configurações avançadas</span><ChevronDown className={`size-4 transition ${advanced ? "rotate-180" : ""}`}/></button>{advanced && <div className="grid gap-4 border-t border-slate-100 p-4 sm:grid-cols-2"><label className={labelClass}>Ordem de exibição<input name="sort_order" defaultValue={plan?.sort_order ?? 0} type="number" min="0" className={inputClass}/></label><label className={labelClass}>Provider <span className="font-normal text-slate-400">(ex.: cakto)<input name="provider" defaultValue={plan?.provider ?? ""} className={inputClass}/></span></label>{([['provider_product_id','Product ID','Identificador do produto no provedor de pagamento.'],['provider_offer_id','Offer ID','Identificador da oferta utilizada na assinatura.'],['provider_checkout_id','Checkout ID',''],['provider_checkout_url','URL do checkout','Copie o link na aba Links do produto na Cakto.']] as const).map(([name, title, help]) => <label key={name} className={`${labelClass} ${name === "provider_checkout_url" ? "sm:col-span-2" : ""}`}>{title}<input name={name} defaultValue={plan?.[name] ?? ""} className={inputClass}/>{help && <span className="mt-1 block text-[11px] font-normal text-slate-400">{help}</span>}</label>)}</div>}</section>

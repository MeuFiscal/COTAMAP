  ...
  return <div className="fixed inset-0 z-50 flex justify-end bg-slate-950/30 backdrop-blur-[2px]" role="dialog" aria-modal="true" aria-label={plan ? `Editar ${plan.name}` : "Novo plano"}>
    <div className="h-full w-full max-w-xl overflow-y-auto bg-[#f8fafc] shadow-2xl">
      ...
      <form onSubmit={submit} className="space-y-7 px-6 py-7 sm:px-8">
        <section><h3 className="text-sm font-black text-slate-950">Informações do plano</h3><div className="mt-4 grid gap-4"><label className={labelClass}>Código interno<input name="code" defaultValue={plan?.code} required pattern="[a-z0-9]+([-_][a-z0-9]+)*" className={inputClass}/></label><label className={labelClass}>Nome<input name="name" value={planName} onChange={e => setPlanName(e.target.value)} required className={inputClass}/></label><label className={labelClass}>Descrição<textarea name="description" defaultValue={plan?.description} rows={2} className={inputClass}/></label></div></section>
        ...
        <section><h3 className="text-sm font-black text-slate-950">Integração Cakto</h3><div className="mt-4 grid gap-4 rounded-2xl border border-orange-100 bg-orange-50/40 p-4"><div className="flex items-end gap-3"><label className={`${labelClass} flex-1`}>Produto Cakto<select value={productId} onChange={e => void loadOffers(e.target.value)} className={inputClass}><option value="">Selecione um produto</option>{products.map(product => <option key={product.id} value={product.id}>{product.name}</option>)}</select></label><button type="button" onClick={() => void refreshProducts()} disabled={catalogLoading} className="mb-0 rounded-xl border border-orange-200 bg-white px-3 py-3 text-xs font-bold text-orange-700">{catalogLoading ? "Atualizando..." : "Atualizar da Cakto"}</button></div><label className={labelClass}>Oferta<select value={offerId} onChange={e => { const selected = offers.find(offer => offer.id === e.target.value); setOfferId(e.target.value); if (selected) { setPlanName(selected.name); setPlanPrice(selected.price ?? ""); } }} disabled={!productId || catalogLoading} className={inputClass}><option value="">Selecione uma oferta</option>{offers.map(offer => <option key={offer.id} value={offer.id}>{offer.name} · {offer.price == null ? "Preço não informado" : `R$ ${offer.price.toFixed(2).replace(".", ",")}`} · {offer.recurrence_period ? `a cada ${offer.recurrence_period} dias` : offer.type || "Oferta"}</option>)}</select></label><p className="text-[11px] text-orange-800/70">O produto é a origem; a oferta define o nome e o preço deste plano. Todas as ofertas do produto são carregadas.</p></div></section>

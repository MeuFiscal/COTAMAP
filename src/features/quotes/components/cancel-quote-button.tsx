...
export function CancelQuoteButton({ requestId }: { requestId: string }) {
  ...
  const mutation = useMutation({ mutationFn: () => cancelQuoteRequest(requestId), onSuccess: () => { void client.invalidateQueries({ queryKey: ["customer-quotations"] }); void client.invalidateQueries({ queryKey: ["quote-request", requestId] }); void client.invalidateQueries({ queryKey: ["quote-notifications", requestId] }); void client.invalidateQueries({ queryKey: ["customer-request-status", requestId] }); void client.invalidateQueries({ queryKey: ["business-calls"] }); } });

...
export default function AdminPage() {
  ...
  return <PrivateShell>
    <nav className="mb-8 flex flex-wrap gap-2" aria-label="Menu administrativo">
      {([["dashboard", "Dashboard"], ["businesses", "Empresas"], ["clients", "Clientes"], ["employees", "Funcionários"], ["audit", "Auditoria"], ["saas", "SaaS"], ["checkouts", "Checkouts"]] as Array<[Tab, string]>).map(([value, label]) => <button key={value} onClick={() => setTab(value)} className={`rounded-xl px-4 py-3 text-sm font-bold ${tab === value ? "bg-[#F97316] text-white" : "bg-white"}`}>{label}</button>)}
    </nav>
    ...
    {tab === "businesses" && <section><h1 className="text-4xl font-black">Empresas</h1><div className="mt-8 space-y-3">{data.businesses.map((business) => <article key={business.id} className="rounded-2xl bg-white p-5"><p className="font-black">{business.name}</p><p className="text-sm text-black/55">{business.city ?? ""}{business.state ? ` · ${business.state}` : ""} · {business.status}</p></article>)}</div><p className="mt-4 text-sm text-black/55">Alterações de status dependem de Edge Functions administrativas ainda não publicadas.</p></section>}
    {tab === "clients" && <section><h1 className="text-4xl font-black">Clientes</h1><div className="mt-8 space-y-3">{data.profiles.filter((profile) => profile.role === "customer").map((profile) => <article key={profile.id} className="rounded-2xl bg-white p-5"><p className="font-black">{profile.full_name}</p><p className="text-sm text-black/55">{profile.email} · {profile.is_active ? "Ativo" : "Inativo"}</p></article>)}</div></section>}
    {tab === "employees" && <section><h1 className="text-4xl font-black">Funcionários</h1><div className="mt-8 space-y-3">{data.employees.map((employee) => <article key={employee.id} className="rounded-2xl bg-white p-5"><p className="font-black">{employee.role}</p><p className="text-sm text-black/55">{employee.presence_status ?? "offline"} · {employee.is_active ? "Ativo" : "Inativo"}</p></article>)}</div></section>}
    {tab === "audit" && <section><h1 className="text-4xl font-black">Auditoria</h1><div className="mt-8 space-y-3">{data.audit.map((entry) => <article key={entry.id} className="rounded-2xl bg-white p-5"><p className="font-black">{entry.action}</p><p className="text-sm text-black/55">{entry.entity_type} · {new Date(entry.created_at).toLocaleString("pt-BR")}</p></article>)}</div></section>}

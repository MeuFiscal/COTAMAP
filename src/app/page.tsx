...
export default async function HomePage() {
  ...
  const [{ data: planRows }, { data: checkout }] = await Promise.all([
    supabase.from("saas_plans").select("id,code,name,description,price,promotional_price,promotion_starts_at,promotion_ends_at,daily_quote_limit,is_unlimited,benefits,provider,provider_product_id,provider_offer_id,provider_checkout_id,provider_checkout_url,sort_order,created_at").eq("is_active", true).eq("is_public", true).order("sort_order", { ascending: true }).order("created_at", { ascending: true }),

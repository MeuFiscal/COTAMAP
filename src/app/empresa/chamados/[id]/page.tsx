...
export default function BusinessCallDetailsPage() {
  ...
  useEffect(() => {
    if (!calls.isLoading && !call) router.replace("/empresa/chamados");
  }, [call, calls.isLoading, router]);
  ...
  if (!call) return <BusinessShell><p className="rounded-2xl bg-[#FFFFFF] p-8">Este chamado foi encerrado ou não está mais disponível. Voltando para chamados ativos...</p></BusinessShell>;

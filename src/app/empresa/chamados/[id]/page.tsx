...
export default function BusinessCallDetailsPage() {
  ...
  const status = useBusinessCallStatus(params.id);
  const queryClient = useQueryClient();
  const [responding, setResponding] = useState(false);
  const [errorMessage, setErrorMessage] = useState<string | null>(null);
  const call = calls.data?.find((item) => item.notification.id === params.id);
  const terminal = status.data == null || status.data.requestStatus !== "waiting" || !["pending", "sent"].includes(status.data.notificationStatus);
  useEffect(() => {
    if (!calls.isLoading && !status.isLoading && terminal) router.replace("/empresa/chamados");
  }, [calls.isLoading, router, status.isLoading, terminal]);

...
export default function OperatorPage() {
  ...
  async function submit() {
    ...
    if (!employee) return;
    setOperator({ id: employee.id, profileId: employee.profile_id, role: employee.role, name: (employee as typeof employee & { full_name?: string | null }).full_name ?? (employee as typeof employee & { email?: string | null }).email ?? "Operador", presenceStatus: employee.presence_status === "online" ? "online" : "offline" });
    router.push("/empresa/dashboard");
  }
  return <section className="mx-auto max-w-2xl space-y-6"><div><p className="text-sm font-bold uppercase tracking-[0.18em] text-[#F97316]">CotaMap Empresa</p><h1 className="mt-2 text-3xl font-black tracking-[-0.04em]">Escolha o usuário</h1><p className="text-black/60">Selecione seu nome e informe o PIN para iniciar.</p></div><div className="grid gap-3">{employees.data?.map((employee) => <button key={employee.id} type="button" onClick={() => { setSelected(employee.id); setError(null); }} className={`rounded-2xl border p-4 text-left ${selected === employee.id ? "border-[#F97316] bg-orange-50" : "border-black/10 bg-white"}`}><span className="font-bold">{(employee as typeof employee & { full_name?: string | null }).full_name ?? (employee as typeof employee & { email?: string | null }).email ?? "Usuário"}</span><span className="mt-1 block text-sm text-black/60">{employee.role === "owner" ? "Assinante" : employee.role === "manager" ? "Gerente" : "Funcionário"} · {employee.presence_status === "online" ? "Online" : "Offline"}</span></button>)}</div>{employees.isLoading ? <p>Carregando usuários...</p> : null}{employees.data?.length === 0 ? <p>Nenhum usuário ativo disponível.</p> : null}{selected ? <div className="space-y-3 rounded-2xl bg-white p-5 shadow-sm"><label htmlFor="operator-pin" className="block text-sm font-bold">PIN do usuário</label><input id="operator-pin" inputMode="numeric" maxLength={6} value={pin} onChange={(event) => setPin(event.target.value.replace(/\D/g, ""))} className="w-full rounded-xl border p-3" placeholder="4 ou 6 dígitos"/><button type="button" onClick={() => void submit()} className="w-full rounded-xl bg-[#F97316] px-4 py-3 font-bold text-white">Entrar</button>{error ? <p className="text-sm text-red-600">{error}</p> : null}</div> : null}</section>;
}
